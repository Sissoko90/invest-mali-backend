...
public class EntrepriseServiceImpl implements EntrepriseService {
    ...
    public Entreprise createEntreprise(EntrepriseRequest req, Utilisateurs createdBy) {
        ...
        for (ParticipantRequest p : req.participants) {
            ...
            // Mettre à jour le nom du cabinet pour les déposants
            if (p.role == EntrepriseRole.DEPOSANT && p.nomCabinet != null && !p.nomCabinet.trim().isEmpty()) {
                System.out.println("✅ [DEBUG] Sauvegarde nom cabinet pour déposant: " + p.nomCabinet);
                person.setNomCabinet(p.nomCabinet.trim());
                personsRepository.save(person);
            }
