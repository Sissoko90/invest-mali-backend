    ...
    public Entreprise createEntreprise(EntrepriseRequest req, Utilisateurs createdBy) {
                    ...
                    for (Entreprise existing : existingEntreprises) {
                        ...
                        if (req.domaineActivite != null && existing.getDomaineActivite() != null && 
                            ...
                            throw new BadRequestException(
                                ...
                                req.domaineActivite + "'. Veuillez choisir un domaine différent pour éviter les conflits."
                        ...
                        if (req.domaineActiviteNr != null && existing.getDomaineActiviteNr() != null && 
                            ...
                            throw new BadRequestException(
                                ...
                                req.domaineActiviteNr + "'. Veuillez choisir un domaine différent pour éviter les conflits."
        ...
        try {
            ...
            if (!foundersEmails.isEmpty()) {
                ...
                // Déterminer le nom à afficher dans l'email

                String displayName = saved.getNom();

                if (displayName == null || displayName.trim().isEmpty()) {

                    // Pour les entreprises individuelles sans nom, utiliser le nom du promoteur

                    displayName = membres.stream()

                        .filter(m -> m.getRole() == EntrepriseRole.PROMOTEUR)

                        .findFirst()

                        .map(m -> {

                            Persons p = m.getPersonne();

                            if (p == null) return "votre entreprise";

                            String prenom = (p.getPrenom() != null && !p.getPrenom().trim().isEmpty()) ? p.getPrenom() : "";

                            String nom = (p.getNom() != null && !p.getNom().trim().isEmpty()) ? p.getNom() : "";

                            String fullName = (prenom + " " + nom).trim();

                            return fullName.isEmpty() ? "votre entreprise" : "Entreprise de " + fullName;

                        })

                        .orElse("votre entreprise");

                }

                

                String subject = "[InvestMali] Création de votre entreprise - " + displayName;
                ...
                bodyBuilder.append("Nous avons le plaisir de vous informer que votre entreprise '").append(displayName).append("' a été créée dans notre système.\n");
