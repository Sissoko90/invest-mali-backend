...
public interface DocumentsRepository extends JpaRepository<Documents, String> {
    ...
    // Recherche par type de document et conjoint
    @Query("SELECT d FROM Documents d WHERE d.typeDocument = :typeDocument AND d.conjoint.id = :conjointId")
    List<Documents> findByTypeDocumentAndConjointId(@Param("typeDocument") TypeDocuments typeDocument, @Param("conjointId") String conjointId);
