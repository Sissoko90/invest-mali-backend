...
const RetraitStep: React.FC<RetraitStepProps> = ({ onDossierUpdate }) => {
  ...
  const handleDownloadDocument = async (entrepriseId: string, documentType: 'RCCM' | 'NINA') => {
    try {
      ...
      console.log(`📄 Documents récupérés pour ${entreprise.nom}:`, documents);
      console.log(`📄 Types de documents disponibles:`, documents.map((d: any) => ({ 
        id: d.id, 
        typeDocument: d.typeDocument, 
        typePiece: d.typePiece,
        numero: d.numero 
      })));
      ...
      if (documentType === 'RCCM') {
        // Chercher le document RCCM uploadé/remplacé à l'étape RCCM2
        // Le backend retourne typeDocument (enum TypeDocuments) qui peut être REGISTRE_COMMERCE
        foundDocument = documents.find((doc: any) => {
          const typeDoc = doc.typeDocument?.toString() || '';
          const isRccmType = typeDoc === 'RCCM' || 
            typeDoc === 'REGISTRE_COMMERCE' ||
            typeDoc.includes('RCCM') ||
            typeDoc.includes('REGISTRE');
          
          console.log(`🔍 Document analysé: id=${doc.id}, typeDocument=${typeDoc}, isRccmType=${isRccmType}`);
        ...
        if (foundDocument) {
          ...
          console.log(`⚠️ Aucun document RCCM/REGISTRE_COMMERCE trouvé parmi les ${documents.length} documents`);
        }
      } else if (documentType === 'NINA') {
        foundDocument = documents.find((doc: any) => 
          doc.typeDocument === 'NINA' || 
      ...
      if (foundDocument) {
        ...
        if (documentType === 'NINA') {
          ...
          // Aucun document RCCM trouvé - afficher un message d'erreur
          throw new Error('Aucun document RCCM n\'a été uploadé pour cette entreprise. Veuillez d\'abord uploader le RCCM à l\'étape RCCM.');
        }
      }
    } catch (err: any) {
      console.error(`❌ Erreur téléchargement ${documentType}:`, err);
