        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-slate-600 font-medium mt-1">
              Renseignez les informations personnelles du {formData.typeEntreprise === 'ENTREPRISE_INDIVIDUELLE' ? 'promoteur' : 'représentant légal'} de l'entreprise.
            </p>
          </div>
        </div>
      </div>
            ...
            <div className="text-center">
              ...
              <div className="mt-4">
                <label htmlFor="registreCommerce" className="cursor-pointer">
                  ...
                  <span className="mt-2 block text-sm font-medium text-slate-600">
                    PDF, DOC, DOCX, JPG, PNG jusqu'é 10MB
                  </span>
                </label>
                <input
                  id="registreCommerce"
        ...
        {formData.typeEntreprise !== 'ENTREPRISE_INDIVIDUELLE' && (
          <div className="bg-white p-6 rounded-lg shadow-md border lg:col-span-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="flex items-center">
                <CheckCircleIcon className={`h-5 w-5 mr-2 ${formData.documents.justificatifDomicile ? 'text-primary-500' : 'text-gray-400'}`} />
                <span className="text-sm">Justificatif domicile</span>
              </div>
            </div>
          </div>
        )}
...
interface FormData {
  // Informations Personnelles (pour entreprises individuelles)
  civilite: Civilites;
  prenom: string;
  nom: string;
  dateNaissance: string;
  ...
  porte: string;
  adresseLibre: string;
  // Informations Déposant (pour sociétés)
  nomDeposant: string;
  ...
  documents: {
    statuts: File | null;
    registreCommerce: File | null;
    justificatifCapital: File | null;
    justificatifDomicile: File | null;
    // Documents spécifiques aux succursales/filiales
    statutsSocieteMere: File | null;
    rccmSocieteMere: File | null;
    pvOuvertureSuccursale: File | null;
    pvDesignationGerant: File | null;
  };
  // Localisation
  division: string;
  antenne: string;
...
const DossierCreationForm: React.FC<DossierCreationFormProps> = ({ onDossierCreated }) => {
  ...
  // Flag pour éviter le reset des sélections lors de la recherche rapide
  const [isApplyingCompanyHierarchy, setIsApplyingCompanyHierarchy] = useState(false);
  const [isApplyingPersonalHierarchy, setIsApplyingPersonalHierarchy] = useState(false);
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    documents: {
      statuts: null,
      registreCommerce: null,
      justificatifCapital: null,
      justificatifDomicile: null,
      // Documents spécifiques aux succursales/filiales
      statutsSocieteMere: null,
      rccmSocieteMere: null,
      pvOuvertureSuccursale: null,
      pvDesignationGerant: null,
    },
    // Localisation
    division: agent?.division || '',
    antenne: agent?.antenne || '',
  ...
  useEffect(() => {
    const loadPersonalArrondissements = async () => {
      ...
      if (personalSelectedRegionId) {
        ...
        // Ne pas réinitialiser les sélections si on applique une hiérarchie depuis la recherche rapide
        if (!isApplyingPersonalHierarchy) {
          setPersonalSelectedCercleId('');
          setPersonalSelectedCommuneId('');
          setPersonalSelectedQuartierId('');
          setPersonalCommunes([]);
        ...
        // Charger les cercles pour toutes les régions (y compris Bamako qui a maintenant un cercle code 9001)
        try {
          const cercles = await divisionService.getCerclesByRegion(personalSelectedRegionId);
          setPersonalCercles(cercles || []);
        } catch (error) {
  ...
  const buildDivisionHierarchy = async (division: any): Promise<any> => {
    ...
    // Note: Bamako a maintenant un cercle (code 9001) selon l'API INSTAT
    // On ne supprime plus le cercle pour Bamako
    
    
    // Détecter si c'est un quartier de Bamako et forcer la reconstruction par code
    const isBamakoQuartier = division.divisionType === 'QUARTIER' && 
                            division.code && 
                            (division.code.startsWith('9001') || ['0001', '0002', '0003', '0004', '0005', '0006', '0007'].some(prefix => division.code.startsWith(prefix)));
  ...
  const reconstructHierarchyByCode = async (division: any): Promise<any> => {
    ...
    // Vérifier si c'est un quartier de Bamako par le code (codes commençant par 9001)
    const isBamakoQuartier = division.divisionType === 'QUARTIER' && division.code && 
        division.code.startsWith('9001');
    
    // Ancienne logique pour les codes 0001-0007
    const isOldBamakoCode = division.divisionType === 'QUARTIER' && division.code && 
        ['0001', '0002', '0003', '0004', '0005', '0006', '0007'].some(prefix => division.code.startsWith(prefix));
    
    if (isBamakoQuartier || isOldBamakoCode) {
      // Trouver la région Bamako (code 90)
      const bamakoRegion = personalRegions.find((r: any) => 
        r.id === '90' || r.code === '90' || 
        (r.nom?.toLowerCase().includes('bamako'))
      );
      ...
      if (bamakoRegion) {
        ...
        // Charger le cercle de Bamako (code 9001)
        try {
          const cercles = await divisionService.getCerclesByRegion(bamakoRegion.id);
          const bamakoCercle = cercles?.find((c: any) => c.code === '9001' || c.nom?.toLowerCase() === 'bamako');
          if (bamakoCercle) {
            hierarchy.cercle = bamakoCercle;
            console.log('? Cercle Bamako trouvé:', bamakoCercle.nom);
            
            // Charger les communes depuis le cercle
            const communes = await divisionService.getCommunesByCercle(bamakoCercle.id);
            console.log('? Communes disponibles:', communes?.map((c: any) => c.code));
            
            // Extraire le code commune depuis le code quartier (ex: 900101010001 -> 90010101)
            // Structure: 90 (région) + 01 (cercle) + 01 (commune) + 01 (?) + 0001 (quartier)
            // Le code commune est les 8 premiers caractères
            const communeCode = division.code.substring(0, 8);
            console.log('? Code commune extrait:', communeCode);
            
            // Chercher la commune correspondante
            let commune = communes?.find((c: any) => c.code === communeCode);
            
            // Si pas trouvé, essayer de trouver par préfixe
            if (!commune) {
              commune = communes?.find((c: any) => division.code.startsWith(c.code));
            }
            ...
            if (commune) {
              ...
              console.log('? Commune non trouvée pour le code:', communeCode);
            }
          }
        } catch (error) {
          console.error('? Erreur chargement cercle/commune Bamako:', error);
        }
      }
    }
  ...
  const applyPersonalHierarchySequential = async (hierarchy: any) => {
    ...
    // Activer le flag pour éviter que le useEffect réinitialise les sélections
    setIsApplyingPersonalHierarchy(true);
    ...
    try {
      ...
      // Désactiver le flag après un court délai pour laisser le temps aux useEffects de se terminer
      setTimeout(() => {
        setIsApplyingPersonalHierarchy(false);
      }, 500);
    }
  };
  ...
  const applyCompanyHierarchySequential = async (hierarchy: any) => {
    ...
    // Activer le flag pour éviter que le useEffect réinitialise les sélections
    setIsApplyingCompanyHierarchy(true);
    ...
    try {
      ...
      // Désactiver le flag après un court délai pour laisser le temps aux useEffects de se terminer
      setTimeout(() => {
        setIsApplyingCompanyHierarchy(false);
      }, 500);
    }
  };
  ...
  const validateStep1 = (): string[] => {
    ...
    // Validation différente selon le type d'entreprise
    if (formData.typeEntreprise === 'SOCIETE') {
      // Pour les sociétés: validation des champs du déposant
      if (!formData.nomDeposant?.trim()) errors.push('Le nom du déposant est obligatoire');
      if (!formData.prenomDeposant?.trim()) errors.push('Le prénom du déposant est obligatoire');
      if (!formData.telephoneDeposant?.trim()) errors.push('Le téléphone du déposant est obligatoire');
    } else {
      // Pour les entreprises individuelles: validation des informations personnelles
      if (!formData.prenom?.trim()) errors.push('Le prénom est obligatoire');
      if (!formData.nom?.trim()) errors.push('Le nom est obligatoire');
      if (!formData.civilite) errors.push('La civilité est obligatoire');
      if (!formData.dateNaissance) {
  ...
  const validateStep3 = (): string[] => {
    ...
    // Validation des participants pour TOUTES les entreprises (étape 3 = Participants)
    if (!formData.participants || formData.participants.length === 0) {
      errors.push('Au moins un participant est obligatoire');
      return errors;
    }
    ...
    formData.participants.forEach((participant, index) => {
      ...
      if (participant.civilite === 'PERSONNE_MORALE') {
        ...
        if (participant.role === 'ADMINISTRATEUR') {
          errors.push(`${participantLabel}: Une personne morale ne peut pas avoir le rôle ADMINISTRATEUR`);
        }
      } else {
        // Validation pour personne physique
        if (!participant.prenom?.trim()) {
      ...
      // Validation du pourcentage de parts
      if (formData.typeEntreprise === 'ENTREPRISE_INDIVIDUELLE') {
      ...
      } else if (formData.typeEntreprise === 'SOCIETE') {
        // Les GIE n'ont pas de parts
        if (formData.formeJuridique !== 'GIE') {
          const isPersonneMorale = participant.civilite === 'PERSONNE_MORALE';
          
          if (roleRequiresParts(participant.role)) {
            if (isPersonneMorale) {
              // Pour les personnes morales, 0% est autorisé mais le champ doit être défini
              if (participant.pourcentageParts === undefined || participant.pourcentageParts === null) {
                errors.push(`${participantLabel}: Le pourcentage de parts doit être défini (0% autorisé pour les personnes morales)`);
              }
            } else {
              // Pour les personnes physiques, le pourcentage doit être > 0
              if (!participant.pourcentageParts || participant.pourcentageParts <= 0) {
                errors.push(`${participantLabel}: Le pourcentage de parts doit être supérieur à 0`);
              }
            }
          }
        }
      ...
      // Validation de l'âge pour les personnes physiques
      if (participant.civilite !== 'PERSONNE_MORALE' && participant.dateNaissance) {
        const age = calculateAge(participant.dateNaissance);
        if (age < 18) {
          errors.push(`${participantLabel}: Doit avoir au moins 18 ans (actuellement: ${age} ans)`);
    ...
    // Vérifier que la somme des pourcentages est 100% (exclure les administrateurs, les entreprises individuelles et les GIE)
    if (formData.typeEntreprise === 'SOCIETE' && formData.formeJuridique !== 'GIE') {
      const totalPercentage = calculateTotalParts();
      if (Math.abs(totalPercentage - 100) > 0.01) {
        errors.push(`La somme des pourcentages doit être exactement 100% (actuellement: ${totalPercentage.toFixed(2)}%) - administrateurs exclus`);
      }
    }
  ...
  const validateStep4 = (): string[] => {
    ...
    // Étape 4 = Documents (pour les sociétés uniquement, les entreprises individuelles sautent cette étape)
    // Validation des documents obligatoires pour les sociétés
    if (formData.typeEntreprise === 'SOCIETE') {
      // Vérifier si c'est une succursale (formes juridiques SUC_*)
      const isSuccursale = formData.formeJuridique?.startsWith('SUC_');
      
      if (isSuccursale) {
        // Documents obligatoires pour les succursales
        if (!formData.documents.pvDesignationGerant) {
          errors.push('Le PV de désignation du gérant Mali est obligatoire pour les succursales');
        }
      } else {
        // Documents obligatoires pour les autres sociétés
        if (!formData.documents.statuts) {
          errors.push('Les statuts de l\'entreprise sont obligatoires');
        }
        if (!formData.documents.registreCommerce) {
          errors.push('Le registre de commerce est obligatoire');
        }
        if (!formData.documents.justificatifCapital) {
          errors.push('Le justificatif de libération de capital est obligatoire');
        }
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log('?? Fichiers é envoyer:', {
        statuts: !!formData.documents.statuts,
        registreCommerce: !!formData.documents.registreCommerce,
        justificatifCapital: !!formData.documents.justificatifCapital,
        justificatifDomicile: !!formData.documents.justificatifDomicile,
        documentsParticipants: formData.participants.map(p => ({
          nom: `${p.prenom} ${p.nom}`,
          documentFile: !!p.documentFile,
      ...
      // Vérifier les documents de l'entreprise pour les sociétés
      if (formData.typeEntreprise === 'SOCIETE') {
        // Vérifier si c'est une succursale ou filiale
        const isSuccursaleFiliale = ['SUC_SARL', 'FIL_SARL', 'SUC_SA', 'FIL_SA', 'SUC_SAS', 'FIL_SAS'].includes(formData.formeJuridique);
        
        if (isSuccursaleFiliale) {
          // Documents spécifiques aux succursales/filiales
          if (!formData.documents.statutsSocieteMere) {
            missingDocs.push('Statuts de la société mère manquants');
          }
          if (!formData.documents.rccmSocieteMere) {
            missingDocs.push('RCCM de la société mère manquant');
          }
          if (!formData.documents.pvOuvertureSuccursale) {
            missingDocs.push('PV assemblée générale - décision ouverture succursale/filiale manquant');
          }
          if (!formData.documents.pvDesignationGerant) {
            missingDocs.push('PV assemblée générale - désignation gérant au Mali manquant');
          }
        } else {
          // Documents standards pour les autres sociétés
          if (!formData.documents.statuts) {
            missingDocs.push('Statuts de l\'entreprise manquants');
          }
          if (!formData.documents.registreCommerce) {
          ...
          // Justificatif de capital requis seulement pour certaines formes juridiques
          // GIE n'en a pas besoin selon la réglementation
          const formesSansJustificatifCapital = ['GIE'];
          const requiresJustificatifCapital = !formesSansJustificatifCapital.includes(formData.formeJuridique);
          
          if (requiresJustificatifCapital && !formData.documents.justificatifCapital) {
            missingDocs.push('Justificatif de libération de capital manquant');
          }
        }
      }
      ...
      // Mode test : permettre la création sans documents pour tester l'API
      const isTestMode = formData.typeEntreprise === 'SOCIETE' 
        ? (!formData.documents.statuts && !formData.documents.registreCommerce && !formData.documents.justificatifCapital)
        : false; // Pas de mode test pour les entreprises individuelles
      ...
      const participantsToCreate = formData.participants.filter(p => {
        ...
        // Vérifier aussi civilite === 'PERSONNE_MORALE' pour compatibilité
        const isPersonneMorale = p.typePersonne === 'MORALE' || p.civilite === 'PERSONNE_MORALE';
        
        // Pour personne morale : vérifier denominationEntreprise
        if (isPersonneMorale) {
          const isValid = p.denominationEntreprise && p.representantLegalNom && p.representantLegalPrenom;
          console.log(`🔍 [TO_CREATE] Personne morale "${p.denominationEntreprise}": valid=${isValid}`);
          return isValid;
        }
        
        // Pour personne physique : vérifier nom et prenom
        if (p.typePersonne === 'PHYSIQUE' || !isPersonneMorale) {
          const isValid = p.nom && p.prenom;
          console.log(`🔍 [TO_CREATE] Personne physique "${p.prenom} ${p.nom}": valid=${isValid}`);
          return isValid;
        }
      ...
      for (const participant of participantsToCreate) {
        ...
        const personRequest = {
          ...
          telephone2: (() => {
            // Gérer le téléphone 2 s'il existe
            if (!participant.telephone2) return undefined; // Retourner undefined au lieu de '' pour éviter la contrainte unique
            
            let phone2 = participant.telephone2.replace(/[\s\-\(\)]/g, '');
            if (!phone2) return undefined; // Si vide après nettoyage, retourner undefined
            ...
            return undefined; // Retourner undefined au lieu de '' pour éviter la contrainte unique
          })(),
          email: (() => {
            const email = participant.email;
            if (!email) return undefined;
      ...
      const allParticipants = formData.participants
        .filter(p => {
          ...
          if (isPersonneMorale) {
            const isValid = p.denominationEntreprise && p.representantLegalNom && p.representantLegalPrenom;
            console.log(`🔍 [FILTER] Personne morale "${p.denominationEntreprise}": valid=${isValid}, id=${p.id}`);
            return isValid;
          } else if (p.typePersonne === 'PHYSIQUE' || p.civilite !== 'PERSONNE_MORALE') {
            const isValid = p.nom && p.prenom;
            console.log(`🔍 [FILTER] Personne physique "${p.prenom} ${p.nom}": valid=${isValid}, id=${p.id}`);
            return isValid;
          }
          return false;
        })
        .map(p => {
          ...
          const isPersonneMorale = p.typePersonne === 'MORALE' || p.civilite === 'PERSONNE_MORALE';
          const participantName = isPersonneMorale ? p.denominationEntreprise : `${p.prenom} ${p.nom}`;
      ...
      const entrepriseRequest = {
        ...
        nomDeposant: formData.typeEntreprise === 'SOCIETE' ? (formData.nomDeposant || '') : null,
        prenomDeposant: formData.typeEntreprise === 'SOCIETE' ? (formData.prenomDeposant || '') : null,
        telephoneDeposant: formData.typeEntreprise === 'SOCIETE' ? (formData.telephoneDeposant || '') : null,
        nomCabinet: formData.typeEntreprise === 'SOCIETE' ? (formData.nomCabinet || '') : null,
        divisionCode: divisionCode,
        participants: (() => {
          // DEBUG: Voir le contenu de allParticipants avant filtrage
          console.log('🔍 DEBUG - allParticipants avant filtrage:', allParticipants.map(p => ({
      ...
      // Upload justificatif de libération de capital (sauf pour GIE et succursales/filiales)
      const formesSansJustificatifCapital = ['GIE', 'SUCCURSALE_FILIALE'];
      const requiresJustificatifCapital = !formesSansJustificatifCapital.includes(formData.formeJuridique);
      
      if (formData.documents.justificatifCapital && gerantId && requiresJustificatifCapital) {
        try { 
          await uploadDocumentFor(gerantId, 'JUSTIFICATIF_CAPITAL', formData.documents.justificatifCapital, `JC-${entrepriseReference}`); 
          console.log('✅ Justificatif de capital uploadé');
        } catch (e) { 
          console.error('❌ Upload justificatif de capital échoué:', e); 
        }
      }
      
      // Upload documents spécifiques aux succursales/filiales
      const isSuccursaleFiliale = ['SUC_SARL', 'FIL_SARL', 'SUC_SA', 'FIL_SA', 'SUC_SAS', 'FIL_SAS'].includes(formData.formeJuridique);
      if (isSuccursaleFiliale && gerantId) {
        // Statuts de la société mère
        if (formData.documents.statutsSocieteMere) {
          try {
            await uploadDocumentFor(gerantId, 'STATUTS_SOCIETE_MERE', formData.documents.statutsSocieteMere, `SSM-${entrepriseReference}`);
            console.log('✅ Statuts société mère uploadés');
          } catch (e) {
            console.error('❌ Upload statuts société mère échoué:', e);
          }
        }
        ...
        if (formData.documents.rccmSocieteMere) {
          try {
            await uploadDocumentFor(gerantId, 'RCCM_SOCIETE_MERE', formData.documents.rccmSocieteMere, `RSM-${entrepriseReference}`);
            console.log('✅ RCCM société mère uploadé');
          } catch (e) {
            console.error('❌ Upload RCCM société mère échoué:', e);
          }
        }
        
        // PV ouverture succursale
        if (formData.documents.pvOuvertureSuccursale) {
          try {
            await uploadDocumentFor(gerantId, 'PV_OUVERTURE_SUCCURSALE', formData.documents.pvOuvertureSuccursale, `PVO-${entrepriseReference}`);
            console.log('✅ PV ouverture succursale uploadé');
          } catch (e) {
            console.error('❌ Upload PV ouverture succursale échoué:', e);
          }
        }
        
        // PV désignation gérant
        if (formData.documents.pvDesignationGerant) {
          try {
            await uploadDocumentFor(gerantId, 'PV_DESIGNATION_GERANT', formData.documents.pvDesignationGerant, `PVG-${entrepriseReference}`);
            console.log('✅ PV désignation gérant uploadé');
          } catch (e) {
            console.error('❌ Upload PV désignation gérant échoué:', e);
          }
        }
      }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h2 className="text-xl font-semibold text-gray-800">
              {formData.typeEntreprise === 'ENTREPRISE_INDIVIDUELLE' ? 'Informations Personnelles' : 'Informations du Déposant'}
            </h2>
            <p className="text-slate-600 font-medium mt-1">
              {formData.typeEntreprise === 'ENTREPRISE_INDIVIDUELLE' 
                ? 'Renseignez les informations personnelles du promoteur de l\'entreprise.'
                : 'Renseignez les informations du déposant et du cabinet.'}
            </p>
          </div>
        </div>
      </div>

      {/* Formulaire simplifié pour les sociétés */}
      {formData.typeEntreprise === 'SOCIETE' ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm font-bold text-slate-700 mb-2">
        ...
        /* Formulaire complet pour les entreprises individuelles */
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      {/* Questions Oui/Non - pour tous les types d'entreprises */}
      <div className="col-span-2 space-y-6 pt-6 border-t border-white/40">
        <div className="flex items-center justify-between mt-4">
          <span className="text-sm font-medium text-gray-700">Avez-vous un extrait de casier judiciaire ?</span>
          <div className="flex space-x-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <div>
                ...
                <select
                    ...
                    disabled={companyCercles.length === 0}
                    className="w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-mali-emerald focus:border-transparent transition-all duration-500 border-gray-300 disabled:bg-gray-100 disabled:cursor-not-allowed"
                  >
                    <option value="">Sélectionnez un cercle</option>
                    {companyCercles.map((cercle: any) => (
  ...
  const ParticipantsStep = () => {
    ...
    const createEmptyParticipant = (personType?: 'PHYSIQUE' | 'MORALE' | null): Participant => {
      const effectiveType = personType ?? selectedPersonType;
      return {
      civilite: effectiveType === 'MORALE' ? 'PERSONNE_MORALE' : (isCreatorFlow ? formData.civilite : ''),
      prenom: isCreatorFlow ? formData.prenom : '',
      nom: isCreatorFlow ? formData.nom : '',
      dateNaissance: isCreatorFlow ? formData.dateNaissance : '',
      lieuNaissance: isCreatorFlow ? formData.lieuNaissance : '',
      ...
      role: formData.typeEntreprise === 'ENTREPRISE_INDIVIDUELLE' 
        ? 'PROMOTEUR' // Pour les entreprises individuelles, toujours PROMOTEUR
        : (effectiveType === 'PHYSIQUE' ? 'GERANT' : 'ASSOCIE'), // Pour les sociétés, logique normale
      pourcentageParts: 0,
      dateDebut: new Date().toISOString().split('T')[0],
      sexe: '',
      situationMatrimoniale: formData.isMarried ? 'MARIE' : 'CELIBATAIRE',
      ...
      paysEmissionRccm: 'MALI',
      rccmFile: undefined,
      typePersonne: effectiveType || 'PHYSIQUE',
      // Champs spécifiques pour les gérants
      hasCriminalRecord: isCreatorFlow ? formData.hasCriminalRecord : undefined,
      casierJudiciaireFile: undefined,
      declarationHonneurFile: undefined,
    ...
    return (
      <div className="space-y-6">
            ...
            <button 
              onClick={() => {
                if (creatorRole === 'GERANT' || creatorRole === 'PROMOTEUR') {
                  // Si gérant/promoteur, ouvrir le formulaire détaillé
                  setSelectedPersonType('PHYSIQUE');
                  setNewParticipant(createEmptyParticipant('PHYSIQUE'));
                  setIsCreatorFlow(true);
                  setEditingIndex(null);
                  setShowDetailedForm(true);
                } else {
          ...
          position: 'relative'
            ...
            {selectedPersonType === 'MORALE' ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {/* Pourcentage de parts - Conditionnel pour personnes morales, masqué pour entreprises individuelles et GIE */}
                {roleRequiresParts(newParticipant.role) && formData.typeEntreprise === 'SOCIETE' && formData.formeJuridique !== 'GIE' ? (
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-2">
                      Pourcentage de parts *
                      {newParticipant.civilite === 'PERSONNE_MORALE' && (
              ...
              {/* Pourcentage de parts - Conditionnel, masqué pour entreprises individuelles et GIE */}
              {roleRequiresParts(newParticipant.role) && formData.typeEntreprise === 'SOCIETE' && formData.formeJuridique !== 'GIE' ? (
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-2">
                    Pourcentage de parts *
              ...
              {(newParticipant.role === 'GERANT' || newParticipant.role === 'PROMOTEUR') && selectedPersonType === 'PHYSIQUE' && (
                <>
                  ...
                  {/* Certificat de résidence - Obligatoire pour tous les étrangers */}
                  {(() => {
                    const participantNationality = newParticipant.nationalite || 'MALIENNE';
                    const isRequired = participantNationality.toUpperCase() !== 'MALIENNE';
                    ...
                    return (
                      <div className="md:col-span-2">
                        ...
                        <p className={`text-xs mt-1 ${
                          ...
                          {isRequired 
                            ? '🏠 Obligatoire pour les étrangers - Formats: PDF, JPG, JPEG, PNG (max 5MB)'
                            : '🏠 Optionnel pour les Maliens - Formats: PDF, JPG, JPEG, PNG (max 5MB)'
                          }
                        </p>
                      </div>
                    );
        ...
        {formData.typeEntreprise !== 'ENTREPRISE_INDIVIDUELLE' && (
          <div className="space-y-4">
            {formData.participants.map((participant, index) => {
              const isPersonneMorale = participant.typePersonne === 'MORALE' || participant.civilite === 'PERSONNE_MORALE';
              return (
              ...
              <div className="space-y-3">
                ...
                <button 
                  onClick={() => {
                    setSelectedPersonType('MORALE');
                    setIsCreatorFlow(false);
                    setNewParticipant(createEmptyParticipant('MORALE'));
                    setEditingIndex(null);
                    setShowPersonTypeModal(false);
                    setShowDetailedForm(true);
                  }}
  ...
  const DocumentsStep = () => (
    <div className="space-y-6">
      ...
      {formData.typeEntreprise !== 'ENTREPRISE_INDIVIDUELLE' && (
        <div className="space-y-6">
          {/* Vérifier si c'est une succursale ou filiale */}
          {['SUC_SARL', 'FIL_SARL', 'SUC_SA', 'FIL_SA', 'SUC_SAS', 'FIL_SAS'].includes(formData.formeJuridique) ? (
            // Documents spécifiques aux succursales/filiales
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="border-2 border-dashed border-white/60 rounded-lg p-8 bg-gradient-to-r from-white/80 via-slate-50/60 to-primary-50/40 shadow-sm hover:shadow-sm transition-all duration-300 hover:border-[#412A5C]/50">
                <div className="text-center">
                  ...
                  <div className="mt-4">
                    <label htmlFor="statutsSocieteMere" className="cursor-pointer">
                      <span className="mt-3 block text-lg font-semibold text-slate-800">
                        Statuts de la société mère *
                      </span>
                      <span className="mt-2 block text-sm font-medium text-slate-600">
                    ...
                    <input
                      id="statutsSocieteMere"
                      name="statutsSocieteMere"
                      type="file"
                      className="sr-only"
                      accept=".pdf,.doc,.docx,.jpg,.jpeg,.png,.webp"
                      onChange={(e) => {
                        const file = e.target.files?.[0] || null;
                        updateFormData('documents', { ...formData.documents, statutsSocieteMere: file });
                      }}
                    />
                  </div>
                  {formData.documents.statutsSocieteMere && (
                    <div className="mt-4 p-3 bg-gradient-to-r from-primary-50 to-primary-50 border border-primary-200 rounded-xl">
                      <p className="text-sm font-bold text-primary-800 flex items-center">
                        <span className="text-lg mr-2">?</span>
                        {formData.documents.statutsSocieteMere.name}
                      </p>
                    </div>
                  )}
                </div>
              ...
              <div className="border-2 border-dashed border-white/60 rounded-lg p-8 bg-gradient-to-r from-white/80 via-slate-50/60 to-primary-50/40 shadow-sm hover:shadow-sm transition-all duration-300 hover:border-[#412A5C]/50">
                <div className="text-center">
                  ...
                  <div className="mt-4">
                    <label htmlFor="pvOuvertureSuccursale" className="cursor-pointer">
                      <span className="mt-3 block text-lg font-semibold text-slate-800">
                        PV décision ouverture succursale *
                      </span>
                      <span className="mt-2 block text-sm font-medium text-slate-600">
                        PDF, DOC, DOCX, JPG, PNG jusqu'à 10MB
                      </span>
                    ...
                    <input
                      id="pvOuvertureSuccursale"
                      name="pvOuvertureSuccursale"
                      type="file"
                      className="sr-only"
                      accept=".pdf,.doc,.docx,.jpg,.jpeg,.png,.webp"
                      onChange={(e) => {
                        const file = e.target.files?.[0] || null;
                        updateFormData('documents', { ...formData.documents, pvOuvertureSuccursale: file });
                      }}
                    />
                  </div>
                  {formData.documents.pvOuvertureSuccursale && (
                    <div className="mt-4 p-3 bg-gradient-to-r from-primary-50 to-primary-50 border border-primary-200 rounded-xl">
                      <p className="text-sm font-bold text-primary-800 flex items-center">
                        <span className="text-lg mr-2">?</span>
                        {formData.documents.pvOuvertureSuccursale.name}
                      </p>
                    </div>
                  )}
                </div>
              ...
              <div className="border-2 border-dashed border-white/60 rounded-lg p-8 bg-gradient-to-r from-white/80 via-slate-50/60 to-primary-50/40 shadow-sm hover:shadow-sm transition-all duration-300 hover:border-[#412A5C]/50">
                <div className="text-center">
                  ...
                  <div className="mt-4">
                    <label htmlFor="pvDesignationGerant" className="cursor-pointer">
                      <span className="mt-3 block text-lg font-semibold text-slate-800">
                        PV désignation gérant Mali *
                      </span>
                      <span className="mt-2 block text-sm font-medium text-slate-600">
                        PDF, DOC, DOCX, JPG, PNG jusqu'à 10MB
                      </span>
                    ...
                    <input
                      id="pvDesignationGerant"
                      name="pvDesignationGerant"
                      type="file"
                      className="sr-only"
                      accept=".pdf,.doc,.docx,.jpg,.jpeg,.png,.webp"
                      onChange={(e) => {
                        const file = e.target.files?.[0] || null;
                        updateFormData('documents', { ...formData.documents, pvDesignationGerant: file });
                      }}
                    />
                  </div>
                  {formData.documents.pvDesignationGerant && (
                    <div className="mt-4 p-3 bg-gradient-to-r from-primary-50 to-primary-50 border border-primary-200 rounded-xl">
                      <p className="text-sm font-bold text-primary-800 flex items-center">
                        <span className="text-lg mr-2">?</span>
                        {formData.documents.pvDesignationGerant.name}
                      </p>
                    </div>
                  )}
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              {/* Justificatif de libération de capital - Conditionnel selon la forme juridique */}
              {(() => {
                const formesSansJustificatifCapital = ['GIE'];
                const requiresJustificatifCapital = !formesSansJustificatifCapital.includes(formData.formeJuridique);
                
                if (!requiresJustificatifCapital) {
                  return null; // Ne pas afficher pour GIE
                }
                ...
                return (
                  <div className="border-2 border-dashed border-white/60 rounded-lg p-8 bg-gradient-to-r from-white/80 via-slate-50/60 to-primary-50/40  shadow-sm hover:shadow-sm transition-all duration-300 hover:border-[#412A5C]/50">
                    <div className="text-center">
                      ...
                      <div className="mt-4">
                        <label htmlFor="justificatifCapital" className="cursor-pointer">
                          <span className="mt-3 block text-lg font-semibold text-slate-800">
                            Justificatif de libération de capital *
                          </span>
                          <span className="mt-2 block text-sm font-medium text-slate-600">
                            PDF, DOC, DOCX, JPG, PNG jusqu'é 10MB
                          </span>
                        ...
                        <input
                          id="justificatifCapital"
                          name="justificatifCapital"
                          type="file"
                          className="sr-only"
                          accept=".pdf,.doc,.docx,.jpg,.jpeg,.png,.webp"
                          onChange={(e) => {
                            const file = e.target.files?.[0] || null;
                            updateFormData('documents', { ...formData.documents, justificatifCapital: file });
                          }}
                        />
                      </div>
                      {formData.documents.justificatifCapital && (
                        <div className="mt-4 p-3 bg-gradient-to-r from-primary-50 to-primary-50 border border-primary-200 rounded-xl">
                          <p className="text-sm font-bold text-primary-800 flex items-center">
                            <span className="text-lg mr-2">?</span>
                            {formData.documents.justificatifCapital.name}
                          </p>
                        </div>
                      )}
                    </div>
      ...
      {formData.typeEntreprise === 'ENTREPRISE_INDIVIDUELLE' && (
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-6">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-lg font-semibold text-amber-800">Entreprise Individuelle</h3>
              <p className="text-amber-700 font-medium">
                Les documents personnels (pièce d'identité, certificat de résidence, casier judiciaire, extrait de naissance) 
                sont requis dans la section Participants. Aucun document d'entreprise supplémentaire n'est nécessaire.
              </p>
            </div>
          </div>
        </div>
  ...
  const RecapitulatifStep = () => (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Informations Personnelles (Entreprise Individuelle) ou Informations du Déposant (Société) */}
          ...
          <div className="space-y-3 text-sm">
            {formData.typeEntreprise === 'ENTREPRISE_INDIVIDUELLE' ? (
              <>
                ...
                <div className="flex justify-between items-center p-2 bg-white/50 rounded-xl border border-white/40">
                  <span className="font-bold text-slate-700">Nom :</span>
                  <span className="font-medium text-slate-600">{formData.nom}</span>
                </div>
                <div className="flex justify-between items-center p-2 bg-white/50 rounded-xl border border-white/40">
                  <span className="font-bold text-slate-700">Prénom :</span>
                  <span className="font-medium text-slate-600">{formData.prenom}</span>
                </div>
                <div className="flex justify-between items-center p-2 bg-white/50 rounded-xl border border-white/40">
                  <span className="font-bold text-slate-700">Date de naissance :</span>
                  <span className="font-medium text-slate-600">{formData.dateNaissance}</span>
              ...
              <>
                ...
                <div className="flex justify-between items-center p-2 bg-white/50 rounded-xl border border-white/40">
                  <span className="font-bold text-slate-700">Nom du Cabinet :</span>
                  <span className="font-medium text-slate-600">{formData.nomCabinet || 'Non renseigné'}</span>
                </div>
              </>
            )}
            <div className="pt-2 border-t border-gray-200">
        ...
        {/* Informations Société/Entreprise */}
        <div className="bg-gradient-to-r from-white/95 via-slate-50/80 to-primary-50/60  p-6 rounded-lg shadow-sm border border-white/60 hover:shadow-sm transition-all duration-300">
          <h3 className="text-lg font-semibold text-slate-800 mb-4 flex items-center">
            <div className="p-2 bg-[#2d85c9] rounded-xl mr-3 shadow-lg">
              <BuildingOfficeIcon className="h-4 w-4 text-white" />
            </div>
            {formData.typeEntreprise === 'ENTREPRISE_INDIVIDUELLE' ? "Informations de l'entreprise" : "Informations de la société"}
          </h3>
          <div className="space-y-3 text-sm">
            <div className="flex justify-between items-center p-2 bg-white/50 rounded-xl border border-white/40">
              <span className="font-bold text-slate-700">Nom :</span>
                  ...
                  <div key={participant.tempId || participant.id || index} className="border-b border-gray-100 pb-2 mb-2 last:border-b-0">
                    {participant.typePersonne === 'MORALE' ? (
                      <>
                        ...
                        <div className="text-primary-600 text-xs">Personne morale</div>
                        <div className="text-gray-600">Représentant: {participant.representantLegalPrenom} {participant.representantLegalNom}</div>
                        <div className="text-gray-600">{participant.role}  {participant.pourcentageParts}% des parts</div>
                        <div className="text-gray-500">Pays RCCM: {participant.paysEmissionRccm}</div>
                      </>
                    ) : (
                      <>
        ...
        {formData.typeEntreprise === 'SOCIETE' && (
          <div className="bg-gradient-to-r from-white/95 via-slate-50/80 to-primary-50/60  p-6 rounded-lg shadow-sm border border-white/60 hover:shadow-sm transition-all duration-300">
            ...
            {['SUC_SARL', 'FIL_SARL', 'SUC_SA', 'FIL_SA', 'SUC_SAS', 'FIL_SAS'].includes(formData.formeJuridique) ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="flex items-center">
                  <CheckCircleIcon className={`h-5 w-5 mr-2 ${formData.documents.statutsSocieteMere ? 'text-primary-500' : 'text-gray-400'}`} />
                  <span className="text-sm">Statuts société mère</span>
                </div>
                <div className="flex items-center">
                  <CheckCircleIcon className={`h-5 w-5 mr-2 ${formData.documents.rccmSocieteMere ? 'text-primary-500' : 'text-gray-400'}`} />
                  <span className="text-sm">RCCM société mère</span>
                </div>
                <div className="flex items-center">
                  <CheckCircleIcon className={`h-5 w-5 mr-2 ${formData.documents.pvOuvertureSuccursale ? 'text-primary-500' : 'text-gray-400'}`} />
                  <span className="text-sm">PV ouverture</span>
                </div>
                <div className="flex items-center">
                  <CheckCircleIcon className={`h-5 w-5 mr-2 ${formData.documents.pvDesignationGerant ? 'text-primary-500' : 'text-gray-400'}`} />
                  <span className="text-sm">PV désignation gérant</span>
                </div>
              </div>
            ) : (
              // Documents standards pour les autres sociétés
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                {/* Justificatif de capital - Conditionnel selon la forme juridique */}
                {(() => {
                  const formesSansJustificatifCapital = ['GIE'];
                  const requiresJustificatifCapital = !formesSansJustificatifCapital.includes(formData.formeJuridique);
                  ...
                  return (
                    <div className="flex items-center">
                      <CheckCircleIcon className={`h-5 w-5 mr-2 ${formData.documents.justificatifCapital ? 'text-primary-500' : 'text-gray-400'}`} />
                      <span className="text-sm">Justificatif de capital</span>
                    </div>
                  );
                })()}
                <div className="flex items-center">
              ...
              <div className="bg-primary-50 p-6 rounded-lg shadow-md border border-primary-200 lg:col-span-2">
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-white p-4 rounded-md border border-primary-200">
                    <h4 className="text-sm font-semibold text-gray-900 mb-2">Prochaines étapes :</h4>
                    <ul className="text-xs text-primary-800 space-y-1">
                      <li>• Dépôt du dossier auprès de l'API-Mali</li>
      ...
      {validationErrors.length > 0 && (
        <div className="mt-6 p-4 bg-red-50 border border-red-200 rounded-lg">
          <div className="flex items-start">
            ...
            <div>
              <h4 className="text-sm font-bold text-red-800 mb-2">Veuillez corriger les erreurs suivantes :</h4>
