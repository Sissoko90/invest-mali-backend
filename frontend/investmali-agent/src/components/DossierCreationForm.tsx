...
const DossierCreationForm: React.FC<DossierCreationFormProps> = ({ onDossierCreated, onClose }) => {
  ...
  // États pour ParticipantsStep - déplacés ici pour survivre aux re-renders
  const [participantEditingIndex, setParticipantEditingIndex] = useState<number | null>(null);
  const [participantCreatorRole, setParticipantCreatorRole] = useState<'GERANT' | 'PROMOTEUR' | 'ASSOCIE' | 'ADMINISTRATEUR'>('GERANT');
  const [showParticipantDetailedForm, setShowParticipantDetailedForm] = useState(false);
  const [showParticipantAddForm, setShowParticipantAddForm] = useState(false);
  const [isParticipantCreatorFlow, setIsParticipantCreatorFlow] = useState(false);
  const [showParticipantPersonTypeModal, setShowParticipantPersonTypeModal] = useState(false);
  const [selectedParticipantPersonType, setSelectedParticipantPersonType] = useState<'PHYSIQUE' | 'MORALE' | null>(null);
  ...
  const getRequiredDocuments = (formeJuridique: FormeJuridique) => {
    ...
    // PV d'Assemblée requis pour SA, SAS, SASU
    if (['SA', 'SAS', 'SASU', 'SUC_SA', 'FIL_SA', 'SUC_SAS', 'FIL_SAS'].includes(formeJuridique)) {
      docs.pvAssemblee = true;
    }

    // Déclaration notariée pour SA, SAS
    if (['SA', 'SAS', 'SUC_SA', 'FIL_SA', 'SUC_SAS', 'FIL_SAS'].includes(formeJuridique)) {
      docs.declarationNotariee = true;
    }

    // Attestation bancaire pour SA, SAS, SARL
    if (['SA', 'SAS', 'SARL', 'SUC_SA', 'FIL_SA', 'SUC_SAS', 'FIL_SAS', 'SUC_SARL', 'FIL_SARL'].includes(formeJuridique)) {
      docs.attestationBancaire = true;
    }

    // RCCM société mère pour succursales et filiales
    if (['SUC_SARL', 'FIL_SARL', 'SUC_SA', 'FIL_SA', 'SUC_SAS', 'FIL_SAS', 'BR'].includes(formeJuridique)) {
      docs.rccmSocieteMere = true;
    }

    return docs;
  };

  const requiredDocs = useMemo(() => getRequiredDocuments(formData.formeJuridique), [formData.formeJuridique]);
  ...
  useEffect(() => {
    ...
    const loadDomaineActivitesNr = async () => {
      ...
      try {
        ...
        console.log('📋 Exemple de données:', response.data.slice(0, 3));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log(`📋 [DEBUG GLOBAL] formData.isMarried: ${formData.isMarried}, formData.conjoints:`, formData.conjoints);
      console.log(`📋 [DEBUG GLOBAL] formData.email: ${formData.email}, formData.prenom: ${formData.prenom}, formData.nom: ${formData.nom}`);
      ...
      for (const participant of participantsToCreate) {
        console.log(`\n🔄 Traitement participant:`, {
          ...
          role: participant.role,
          situationMatrimoniale: participant.situationMatrimoniale,
          hasConjoints: participant.conjoints && participant.conjoints.length > 0
        });
        ...
        // Utiliser l'email comme identifiant unique car nom/prénom peuvent avoir des variations

        const isCreatorParticipant = participant.email === formData.email &&

                                   (participant.role === 'GERANT' || participant.role === 'PROMOTEUR');
        
        console.log(`🔍 Vérification créateur pour ${participant.prenom} ${participant.nom}:`, {
          participantEmail: participant.email,
          formDataEmail: formData.email,
          isCreator: isCreatorParticipant,
          role: participant.role,
          formDataIsMarried: formData.isMarried,
          formDataConjointsLength: formData.conjoints ? formData.conjoints.length : 0,
          participantSituationMatrimoniale: participant.situationMatrimoniale,
          participantConjointsLength: participant.conjoints ? participant.conjoints.length : 0
        });
        ...
        const personRequest = {
          ...
          // Conjoints (pour tous les participants mariés)
          // Si c'est le créateur et qu'il est marié, utiliser formData.conjoints
          // Sinon, utiliser participant.conjoints s'il existe

          conjoints: (() => {
            // 1. Si le participant a déjà des conjoints définis, les utiliser
            if (participant.conjoints && participant.conjoints.length > 0) {
              console.log(`💍 Ajout de ${participant.conjoints.length} conjoint(s) pour ${participant.prenom} ${participant.nom} (depuis participant.conjoints)`);
              return participant.conjoints.map(c => ({
                prenom: c.prenom,
                nom: c.nom,
            ...
            // 2. Si c'est le créateur marié ET qu'il n'a pas de conjoints définis, utiliser formData.conjoints
            if (isCreatorParticipant && formData.isMarried && formData.conjoints && formData.conjoints.length > 0) {
              console.log(`💍 Ajout de ${formData.conjoints.length} conjoint(s) pour le créateur ${participant.prenom} ${participant.nom} (depuis formData.conjoints)`);
              return formData.conjoints.map(c => ({
                prenom: c.prenom,
                nom: c.nom,
                dateMariage: c.dateMariage,
                lieuMariage: c.lieuMariage,
            ...
            // 3. Si le participant est marié mais n'a pas de conjoints, vérifier s'il correspond au créateur par nom/prénom
            if (participant.situationMatrimoniale === 'MARIE' && 
                formData.isMarried && 
                formData.conjoints && 
                formData.conjoints.length > 0 &&
                participant.prenom === formData.prenom && 
                participant.nom === formData.nom) {
              console.log(`💍 Ajout de ${formData.conjoints.length} conjoint(s) pour ${participant.prenom} ${participant.nom} (correspondance nom/prénom avec créateur)`);
              return formData.conjoints.map(c => ({
                prenom: c.prenom,
                nom: c.nom,
                dateMariage: c.dateMariage,
            ...
            console.log(`⚠️ Aucun conjoint trouvé pour ${participant.prenom} ${participant.nom} (marié: ${participant.situationMatrimoniale === 'MARIE'})`);
            return [];
          })()
        ...
        console.log(`💍 Conjoints envoyés pour ${participant.prenom} ${participant.nom}:`, personRequest.conjoints);
        
        // IMPORTANT: Mettre à jour formData.participants avec les conjoints pour qu'ils soient disponibles lors du mapping final
        if (personRequest.conjoints && personRequest.conjoints.length > 0) {
          participant.conjoints = personRequest.conjoints.map((c, idx) => ({
            id: `conjoint-${Date.now()}-${idx}`,
            ...c
          }));
          console.log(`✅ Conjoints sauvegardés dans formData.participants pour ${participant.prenom} ${participant.nom}`);
        }
        ...
        try {
          ...
          try {
            ...
            if (searchResponse.ok) {
              ...
              // IMPORTANT: Sauvegarder les conjoints dans participant AVANT le continue
              if (personRequest.conjoints && personRequest.conjoints.length > 0) {
                participant.conjoints = personRequest.conjoints.map((c, idx) => ({
                  id: `conjoint-${Date.now()}-${idx}`,
                  ...c
                }));
                console.log(`✅ Conjoints sauvegardés dans formData.participants pour participant existant ${participant.prenom} ${participant.nom}`);
              }
      ...
      const entrepriseRequest = {
        ...
        extraitJudiciaire: (() => {
          // Vérifier si au moins un gérant/promoteur a un casier judiciaire
          const hasGerantWithCasier = formData.participants.some(p => 
            (p.role === 'GERANT' || p.role === 'PROMOTEUR') && 
            p.hasCriminalRecord === true
          );
          return hasGerantWithCasier || formData.hasCriminalRecord || false;
        })(),
        ...
        participants: (() => {
          ...
          const participantsWithPersonalData = realParticipants.map(participant => {
            ...
            if (originalParticipant) {
              ...
              // Ajouter conjoints si disponible

              if (originalParticipant.conjoints && originalParticipant.conjoints.length > 0) {

                result.conjoints = originalParticipant.conjoints.map(c => ({
                ...
                console.log(`💍 [PARTICIPANT] ${participant.personId} - ${result.conjoints.length} conjoint(s) ajouté(s)`);
      ...
      const uploadPieceForParticipant = async (personId: string, typePiece: string, numeroPiece: string | null, file: File) => {
        ...
        // N'ajouter le numéro que s'il est fourni (éviter d'envoyer "null" comme string)
        if (numeroPiece) {
        ...
        try {
          ...
          console.error('❌ [UPLOAD PIECE] PersonId:', personId, 'EntrepriseId:', entrepriseId, 'TypePiece:', typePiece);
      ...
      const uploadDocumentFor = async (personId: string, typeDocument: string, file: File, numero?: string, conjointId?: string) => {
        ...
        try {
          ...
          console.error('❌ [UPLOAD DOCUMENT] PersonId:', personId, 'EntrepriseId:', entrepriseId, 'TypeDocument:', typeDocument);
      ...
      for (const participant of formData.participants) {
        ...
        if (participant.civilite === 'PERSONNE_MORALE') {
          ...
          // Upload des pièces d'identité (CNI, PASSEPORT, etc.) pour les personnes physiques
          console.log('🔍 [PIECE IDENTITE] Participant:', participant.prenom, participant.nom);
          console.log('🔍 [PIECE IDENTITE] participant.id:', participant.id);
          console.log('🔍 [PIECE IDENTITE] participant.documentFile:', !!participant.documentFile, participant.documentFile?.name);
          console.log('🔍 [PIECE IDENTITE] participant.typePiece:', participant.typePiece);
          console.log('🔍 [PIECE IDENTITE] participant.numeroPiece:', participant.numeroPiece);

          // Le numéro de pièce est optionnel, on envoie null si non renseigné
          if (participant.id && participant.documentFile && participant.typePiece) {

            try {

              // Utiliser null si le numéro n'est pas renseigné (évite les faux numéros en base)
              const numeroPiece = participant.numeroPiece || null;
              console.log(`📄 Upload pièce ${participant.typePiece} pour ${participant.prenom} ${participant.nom} (numéro: ${numeroPiece || 'non renseigné'})`);

              await uploadPieceForParticipant(participant.id, participant.typePiece, numeroPiece, participant.documentFile);
              
              console.log(`✅ Pièce ${participant.typePiece} uploadée avec succès pour ${participant.prenom} ${participant.nom}`);
            ...
            console.warn('⚠️ [PIECE IDENTITE] Conditions non remplies pour', participant.prenom, participant.nom);
            console.warn('⚠️ [PIECE IDENTITE] Manquant:', {
              id: !participant.id,
              documentFile: !participant.documentFile,
              typePiece: !participant.typePiece
            });
      ...
      // Certificat de nationalité (PIECE_NATIONALITE) pour les entreprises individuelles
      // Note: C'est un DOCUMENT (TypeDocuments), pas une pièce d'identité (TypePieces)
      console.log('🔍 [CERTIFICAT NATIONALITE] Type entreprise:', formData.typeEntreprise);
      console.log('🔍 [CERTIFICAT NATIONALITE] gerantParticipant:', gerantParticipant);
      console.log('🔍 [CERTIFICAT NATIONALITE] gerantId:', gerantId);
      
      if (formData.typeEntreprise === 'ENTREPRISE_INDIVIDUELLE') {

        const certificatNationalite = gerantParticipant?.pieceNationaliteFile;
        console.log('🔍 [CERTIFICAT NATIONALITE] Fichier présent:', !!certificatNationalite, certificatNationalite?.name);

        

        if (certificatNationalite && gerantId) {

          try { 

            console.log('📤 [CERTIFICAT NATIONALITE] Upload en cours pour gerantId:', gerantId);
            // Upload comme document PIECE_NATIONALITE
            await uploadDocumentFor(gerantId, 'PIECE_NATIONALITE', certificatNationalite, `CN-${entrepriseReference}`);

            console.log(`✅ Certificat de nationalité uploadé pour l'entreprise individuelle`);

          } catch (e) {

            console.error('❌ Upload certificat de nationalité échoué:', e);

          }

        } else {
          console.warn('⚠️ [CERTIFICAT NATIONALITE] Conditions non remplies - certificatNationalite:', !!certificatNationalite, 'gerantId:', gerantId);
        }
      ...
      // Nettoyer le localStorage après la création réussie
      console.log('🧹 Nettoyage du localStorage...');
      localStorage.removeItem('dossier_creation_cache');
      console.log('✅ localStorage nettoyé');

      // Réinitialiser complètement le formulaire immédiatement
        ...
        <div className="space-y-6">
          ...
          <div className="relative" ref={domaineDropdownRef}>
            ...
            <label className="block text-sm font-bold text-slate-700 mb-2">
              ...
              Domaine d'activité {domaineActiviteNrOptions.length > 0 && `(${domaineActiviteNrOptions.length} options disponibles)`}
  ...
  const ParticipantsStep = () => {
    ...
    // Utiliser les états du parent au lieu de créer des états locaux
    const editingIndex = participantEditingIndex;
    const setEditingIndex = setParticipantEditingIndex;
    const creatorRole = participantCreatorRole;
    const setCreatorRole = setParticipantCreatorRole;
    const showDetailedForm = showParticipantDetailedForm;
    const setShowDetailedForm = setShowParticipantDetailedForm;
    const showAddForm = showParticipantAddForm;
    const setShowAddForm = setShowParticipantAddForm;
    const isCreatorFlow = isParticipantCreatorFlow;
    const setIsCreatorFlow = setIsParticipantCreatorFlow;
    const showPersonTypeModal = showParticipantPersonTypeModal;
    const setShowPersonTypeModal = setShowParticipantPersonTypeModal;
    const selectedPersonType = selectedParticipantPersonType;
    const setSelectedPersonType = setSelectedParticipantPersonType;



    const createEmptyParticipant = useCallback((): Participant => ({
    ...
    }), [selectedPersonType, isCreatorFlow, formData.civilite, formData.prenom, formData.nom, formData.dateNaissance, formData.lieuNaissance, formData.nationalite, formData.telephonePersonnel, formData.telephonePersonnel2, formData.emailPersonnel, formData.adressePersonnelle, formData.typeEntreprise, formData.isMarried, formData.hasCriminalRecord]);



    // newParticipant et setNewParticipant sont maintenant dans le parent
    ...
    // useRef pour gérer le champ numeroPiece sans re-render
    const numeroPieceRef = React.useRef<HTMLInputElement>(null);

    // Synchroniser le champ numeroPiece quand le participant change
    React.useEffect(() => {
      if (numeroPieceRef.current && numeroPieceRef.current !== document.activeElement) {
        numeroPieceRef.current.value = newParticipant.numeroPiece || '';
      }
    }, [newParticipant.id, editingIndex]);
    ...
    useEffect(() => {
      ...
      if (selectedPersonType && editingIndex === null) {
        // Ne pas réinitialiser si l'utilisateur a déjà commencé à remplir le formulaire
        const hasStartedFilling = newParticipant.sexe || newParticipant.typePiece || newParticipant.numeroPiece || newParticipant.documentFile;
        if (!hasStartedFilling) {
          setNewParticipant(createEmptyParticipant());
        }
      }

    }, [selectedPersonType, editingIndex, newParticipant.sexe, newParticipant.typePiece, newParticipant.numeroPiece, newParticipant.documentFile]);
    ...
    }, [isCreatorFlow, editingIndex, newParticipant.sexe, newParticipant.typePiece, newParticipant.numeroPiece, newParticipant.documentFile]);
    ...
    const addParticipant = () => {
      console.log('🔍 [ADD PARTICIPANT] newParticipant avant ajout:', newParticipant);
      ...
      if (editingIndex !== null) {
        ...
        console.log('🔍 [ADD PARTICIPANT] participant à ajouter:', participant);
          ...
          <div className="bg-gradient-to-r from-white/95 border border-white/60 rounded-lg p-6 mb-6 shadow-sm">
            ...
            {selectedPersonType === 'MORALE' ? (
              ...
              <div>
                ...
                <label htmlFor="numeroPiece" className="block text-sm font-bold text-slate-700 mb-2">Numéro de la pièce (optionnel)</label>

                <input

                  key={`numeroPiece-${newParticipant.id || editingIndex || 'new'}`}
                  ...
                  defaultValue={newParticipant.numeroPiece || ''}

                  onBlur={(e) => setNewParticipant({...newParticipant, numeroPiece: e.target.value})}
              ...
              {(newParticipant.role === 'GERANT' || newParticipant.role === 'PROMOTEUR') && selectedPersonType === 'PHYSIQUE' && (
                ...
                <>
                  ...
                  {(() => {
                    const isCreator = newParticipant.email === formData.email || 
                                     (formData.typeEntreprise === 'ENTREPRISE_INDIVIDUELLE' && 
                                      (newParticipant.role === 'GERANT' || newParticipant.role === 'PROMOTEUR'));
                    return isCreator && formData.isMarried && formData.conjoints && formData.conjoints.length > 0;
                  })() && (
            ...
            <div className="pt-2 border-t border-gray-200">
              ...
              <span className="font-medium text-slate-600">
                {(() => {
                  ...
                  const hasGerantWithCasier = formData.participants.some(p => 
                    (p.role === 'GERANT' || p.role === 'PROMOTEUR') && 
                    (p.hasCriminalRecord === true || p.casierJudiciaireFile)
                  );
                  return (hasGerantWithCasier || formData.hasCriminalRecord) ? 'Oui' : 'Non';
                })()}
              </span>
              ...
              <span className="font-medium text-slate-600">
                {(() => {
                  // Vérifier si au moins un participant est marié ou si le créateur est marié
                  const hasMarriedParticipant = formData.participants.some(p => 
                    p.situationMatrimoniale === 'MARIE'
                  );
                  return (hasMarriedParticipant || formData.isMarried) ? 'Oui' : 'Non';
                })()}
              </span>
