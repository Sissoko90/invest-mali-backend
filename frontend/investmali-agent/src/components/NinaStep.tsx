...
const NinaStep: React.FC<NinaStepProps> = ({ onEntrepriseUpdate }) => {
  ...
  const getStatusBadge = (entreprise: Entreprise) => {
    if (entreprise.numeroNina) {
      return (
        <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-sky-100 text-blue-800">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex flex-wrap gap-2">
                            <button
                              ...
                              className="inline-flex items-center px-4 py-2 border border-blue-300 text-lg font-medium rounded-md text-blue-700 bg-sky-50 hover:bg-sky-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
...
const NinaStep: React.FC<NinaStepProps> = ({ onEntrepriseUpdate }) => {
  ...
  const getPhoneNumber2 = (entreprise: Entreprise): string => {
    // Chercher le gérant ou promoteur dans les membres de l'entreprise
    const gerant = membresEntreprise.find(membre => 
      membre.role === 'GERANT' || membre.role === 'PROMOTEUR'
    );
    
    if (gerant && gerant.telephone2) {
      console.log('✅ Téléphone2 du gérant trouvé:', gerant.telephone2);
      return gerant.telephone2;
    }
    
    // Retourner une chaîne vide si pas de téléphone2
    return '';
  };
              ...
              {(() => {
                ...
                return (
                  <Certificate
                    ...
                    adresseSiege={getAdresseSiege(entrepriseToUse)}
                    telephone1={getPhoneNumber(entrepriseToUse)}
                    telephone2={getPhoneNumber2(entrepriseToUse)}
                    dateDemande={dateDemande}
                    dateSignature={dateSignature}
                    signatureFile={signatureFile}
                    signaturePreview={signaturePreview}
