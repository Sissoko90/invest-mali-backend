...
const TCOMStep: React.FC<TCOMStepProps> = ({ onDossierUpdate }) => {
  ...
  const handleFinaliserTCOM = async (demandeId: string, decision: 'approuve' | 'rejete', commentaire?: string) => {
    try {
      ...
      if (decision === 'approuve') {
        console.log(`✅ [TCOMStep] Dossier approuvé avec succès - Transition: TCOM → RCCM`);
      } else {
        console.log(`❌ [TCOMStep] Dossier rejeté - Retour à l'étape précédente - Raison: ${commentaire}`);
      }
