...
const ParticipantsStep: React.FC<ParticipantsStepProps> = ({ data, updateData, onNext }) => {
  ...
  const [showUserRoleForm, setShowUserRoleForm] = useState(false);
  // Rôle par défaut selon le type d'entreprise
  const defaultRole = data.companyInfo?.typeEntreprise === 'ENTREPRISE_INDIVIDUELLE' ? 'GERANT' : 'ASSOCIE';
  const [userRole, setUserRole] = useState<EntrepriseRole>(defaultRole);
  const [formData, setFormData] = useState<Participant>({
    nom: '',
    prenom: '',
  ...
  // Mettre à jour le rôle par défaut quand le type d'entreprise change
  React.useEffect(() => {
    const newDefaultRole = data.companyInfo?.typeEntreprise === 'ENTREPRISE_INDIVIDUELLE' ? 'GERANT' : 'ASSOCIE';
    setUserRole(newDefaultRole);
    setFormData(prev => ({ ...prev, role: newDefaultRole, pourcentageParts: newDefaultRole === 'GERANT' && data.companyInfo?.typeEntreprise === 'ENTREPRISE_INDIVIDUELLE' ? 100 : prev.pourcentageParts }));
  }, [data.companyInfo?.typeEntreprise]);
  ...
  const validateParticipants = (): string[] => {
    ...
    if (isEntrepriseIndividuelle) {
      ...
      const nonDirigeants = participants.filter(p => p.role !== 'GERANT');
      if (nonDirigeants.length > 0) {
        validationErrors.push('Pour une entreprise individuelle, le seul rôle autorisé est "Gérant"');
      }

      // 3. Le dirigeant doit avoir 100% des parts
      const dirigeant = participants.find(p => p.role === 'GERANT');
      if (dirigeant && Math.abs(dirigeant.pourcentageParts - 100) > 1) {
        validationErrors.push('Le gérant d\'une entreprise individuelle doit avoir 100% des parts');
      }
      ...
      participants.forEach((p, idx) => {
        ...
        if (!p.typePiece || !p.documentFile) {
          validationErrors.push(`${label}: type de pièce et document sont obligatoires`);
        }
    ...
    const totalParts = participants
      .filter(p => p.role === 'GERANT' || p.role === 'ASSOCIE' || p.role === 'GERANT')
      .reduce((sum, p) => sum + p.pourcentageParts, 0);
  ...
  const handleAddParticipant = () => {
    ...
    const isEntrepriseIndividuelle = data.companyInfo?.typeEntreprise === 'ENTREPRISE_INDIVIDUELLE';
    const requiresManagerDocuments = formData.role === 'GERANT';
    ...
    const newParticipant: Participant = { 
      ...
      situationMatrimoniale: (formData.role === 'GERANT')
        ? (data.personalInfo?.isMarried ? 'MARIE' : 'CELIBATAIRE')
        : formData.situationMatrimoniale
    };
    const updatedParticipants = [...(data.participants || []), newParticipant];
  ...
  const handleAddUserAsParticipant = () => {
    ...
    console.log('🔍 DEBUG - Champs personnels récupérés pour participant:', {
      'isForSelf': data.personalInfo?.isForSelf,
    ...
    // Valider et nettoyer le rôle utilisateur
    const validRoles: EntrepriseRole[] = ['GERANT', 'ASSOCIE'];
    const cleanUserRole = userRole?.toString().trim().toUpperCase() as EntrepriseRole;
    ...
    // Déterminer la source des données selon isForSelf
    const isForSelf = data.personalInfo?.isForSelf;
    console.log('🔍 DEBUG - Source des données:', isForSelf ? 'Données utilisateur récupérées automatiquement' : 'Données saisies manuellement');

    let participantData: Participant;

    if (isForSelf) {
      // Si "Oui, c'est pour moi" : utiliser les données récupérées automatiquement
      console.log('✅ Utilisation des données utilisateur récupérées automatiquement');
      participantData = {
        personId: currentUser.personne_id,
      ...
      // Si "Non, c'est pour quelqu'un d'autre" : utiliser les données saisies manuellement
      console.log('✅ Utilisation des données saisies manuellement dans le formulaire');
      participantData = {
        personId: undefined, // Pas de personId car c'est une nouvelle personne
        nom: data.personalInfo?.lastName || '',
        prenom: data.personalInfo?.firstName || '',
    ...
    console.log('🔍 DEBUG - Participant créé avec les données:', {
      'source': isForSelf ? 'automatique' : 'manuelle',
      'nom': participantData.nom,
      'prenom': participantData.prenom,
      'role': participantData.role,
      'lieuNaissance': participantData.lieuNaissance,
      'nationnalite': participantData.nationnalite,
      'divisionId': participantData.divisionId,
      'localite': participantData.localite
    });
  ...
  const handleAutoDistributeParts = () => {
    ...
    const eligibleParticipants = data.participants.filter(p => 
      p.role === 'GERANT' || p.role === 'ASSOCIE' || p.role === 'GERANT'
    );
    ...
    const updatedParticipants = data.participants.map(participant => {
      if (participant.role === 'GERANT' || participant.role === 'ASSOCIE' || participant.role === 'GERANT') {
        return {
          ...participant,
          pourcentageParts: Math.round((participant.pourcentageParts + partsPerParticipant) * 100) / 100
        };
          ...
          {data.personalInfo?.isForSelf ? (
            <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-4">
              <div className="flex items-center mb-2">
                ...
                <span className="text-sm font-medium text-green-800">Données automatiques</span>
              </div>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-4">
              <div className="flex items-center mb-2">
                ...
                <span className="text-sm font-medium text-amber-800">Données saisies manuellement</span>
              </div>
                ...
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    ...
                    <div className="bg-white rounded-lg p-3 mb-3 border border-gray-200">
                      <h5 className="text-sm font-medium text-gray-700 mb-2">Informations personnelles</h5>
          ...
          <div className="border-t pt-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div>
                ...
                {data.companyInfo?.typeEntreprise === 'ENTREPRISE_INDIVIDUELLE' && (
                  <p className="text-sm text-gray-500 mt-1">
                    Le rôle est fixé à "Gérant" pour une entreprise individuelle
                  </p>
                )}
              </div>
              ...
              {(formData.role === 'GERANT') && data.personalInfo?.hasCriminalRecord && (
                <div className="md:col-span-2">
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Casier judiciaire *
                  </label>
              ...
              {(formData.role === 'GERANT') && data.personalInfo?.isMarried && (
                <div className="md:col-span-2">
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Acte de mariage *
                  </label>
