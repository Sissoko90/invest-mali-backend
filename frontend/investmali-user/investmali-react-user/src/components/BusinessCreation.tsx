...
const ensureAdultBirthDate = (birthDate: string | null | undefined): string => {
  ...
  if (isNaN(birth.getTime())) {
    ...
    console.warn('⚠️ Date de naissance invalide:', birthDate);
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (file) {
                      ...
                      // Ne pas utiliser spread operator pour préserver le type File

                      const updatedConjoint = newConjoints[index];

                      updatedConjoint.acteMariageFile = file;

                      updatedConjoint.acteMariageFilename = file.name;
    ...
    if (personal?.isMarried === true) {
      ...
      if (!personal.conjoints || personal.conjoints.length === 0) {
        ...
        personal.conjoints.forEach((conjoint, index) => {
          ...
          if (!conjoint.acteMariageFile) personalErrors.push(`${conjointLabel}: Acte de mariage obligatoire`);
      ...
      participants.forEach((p, index) => {
        ...
        if (p.civilite === 'PERSONNE_MORALE') {
          ...
          if (p.role === 'GERANT' || p.role === 'PROMOTEUR') {
            ...
            // Vérifier isMarried dans personalInfo au lieu du participant

            if (data.personalInfo?.isMarried === true && data.personalInfo?.conjoints && data.personalInfo.conjoints.length > 0) {
  ...
  const handleSubmitEntreprise = async () => {
    ...
    try {
      ...
      (data.participants || []).forEach((p, idx) => {
        ...
        if (requiresManagerDocuments && data.personalInfo?.isMarried === true && data.personalInfo?.conjoints && data.personalInfo.conjoints.length > 0) {
      ...
      const uploadDocumentFor = async (personId: string, typeDocument: string, file: File, numero?: string, conjointId?: string) => {
        ...
        if (!file) {
          ...
          console.error(`❌ [UPLOAD ERROR] ${typeDocument}: Fichier null ou undefined`);

          throw new Error(`Fichier ${typeDocument} manquant`);
        ...
        if (!file.name || typeof file.size !== 'number') {
          ...
          console.error(`❌ [UPLOAD ERROR] ${typeDocument}: Fichier invalide (pas de name ou size)`, file);

          throw new Error(`Fichier ${typeDocument} invalide`);
      ...
      if (gerantId) {
        ...
        if (data.personalInfo?.isMarried && data.personalInfo?.conjoints && data.personalInfo.conjoints.length > 0) {
          ...
          try {
              ...
              if (personData.situationMatrimoniale !== 'MARIE') {
                ...
                try {
                  ...
                  if (updateResponse.ok) {
                    ...
                    console.log('✅ Statut matrimonial mis à jour vers MARIE');

                  } else {

                    const errorText = await updateResponse.text();

                    console.error('❌ Erreur mise à jour statut matrimonial:', errorText);

                  }

                } catch (e) {

                  console.error('❌ Erreur lors de la mise à jour du statut matrimonial:', e);
            ...
            console.error('❌ Erreur lors de la récupération des données de la personne:', e);

          }

          

          // Créer les conjoints en base de données et uploader leurs actes de mariage

          for (const conjoint of data.personalInfo.conjoints) {

            console.log(`🔍 [DEBUG CONJOINT] Traitement du conjoint ${conjoint.prenom} ${conjoint.nom}`);

            

            try {

              // 1. Créer le conjoint en base de données
              ...
              const conjointData = {
                ...
                dateMariage: conjoint.dateMariage,

                lieuMariage: conjoint.lieuMariage,

                regimeMatrimonial: conjoint.regimeMatrimonial,

                clauseRestrictive: conjoint.clauseRestrictive

              };

              

              console.log(`📝 [CONJOINT] Création du conjoint en base:`, conjointData);

              

              const createConjointResponse = await fetch(`/api/v1/conjoints/person/${gerantId}`, {
                ...
                body: JSON.stringify(conjointData)

              });

              

              if (!createConjointResponse.ok) {

                const errorText = await createConjointResponse.text();

                console.error(`❌ Erreur création conjoint:`, errorText);

                throw new Error(`Échec de la création du conjoint ${conjoint.prenom} ${conjoint.nom}`);

              }

              

              const createdConjoint = await createConjointResponse.json();

              const conjointId = createdConjoint.id;

              console.log(`✅ Conjoint créé avec l'ID: ${conjointId}`);

              

              // 2. Uploader l'acte de mariage si présent

              if (conjoint.acteMariageFile) {

                // Vérifier que le fichier a les propriétés nécessaires (name, size)

                // Note: instanceof File peut échouer si l'objet a été sérialisé/désérialisé

                if (!conjoint.acteMariageFile.name || typeof conjoint.acteMariageFile.size !== 'number') {

                  console.error(`❌ acteMariageFile n'a pas les propriétés requises pour ${conjoint.prenom} ${conjoint.nom}:`, conjoint.acteMariageFile);

                  throw new Error(`Le fichier de l'acte de mariage pour ${conjoint.prenom} ${conjoint.nom} n'est pas valide`);

                }

                

                console.log(`📤 [ACTE MARIAGE] Upload pour le conjoint ${conjointId}`, {

                  fileName: conjoint.acteMariageFile.name,

                  fileSize: conjoint.acteMariageFile.size,

                  fileType: conjoint.acteMariageFile.type

                });

                

                // Uploader via l'endpoint /documents/document avec le conjointId

                const formData = new FormData();

                formData.append('personneId', gerantId);

                formData.append('entrepriseId', entrepriseId);

                formData.append('typeDocument', 'ACTE_MARIAGE');

                formData.append('numero', `AM-${conjoint.prenom}-${conjoint.nom}`);

                formData.append('conjointId', conjointId);

                formData.append('file', conjoint.acteMariageFile);

                

                const uploadActeResponse = await fetch(`/api/v1/documents/document`, {
                ...
                if (!uploadActeResponse.ok) {

                  const errorText = await uploadActeResponse.text();

                  console.error(`❌ Erreur upload acte de mariage:`, errorText);

                  throw new Error(`Échec de l'upload de l'acte de mariage pour ${conjoint.prenom} ${conjoint.nom}`);
                ...
                console.warn(`⚠️ Aucun acte de mariage trouvé pour ${conjoint.prenom} ${conjoint.nom}`);
              ...
              console.error(`❌ Erreur traitement conjoint ${conjoint.prenom} ${conjoint.nom}:`, e);

              const errorMessage = e instanceof Error ? e.message : String(e);

              throw new Error(`Échec du traitement du conjoint ${conjoint.prenom} ${conjoint.nom}: ${errorMessage}`);
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium text-investmali-neutral-dark">
                  ...
                  {companyLocationName || 
                   ...
                   (data.companyInfo?.divisionCode && !data.companyInfo.divisionCode.match(/^\d+$/) 
...
const BusinessCreation: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchDivisionNames = async () => {
      ...
      if (businessData.companyInfo?.selectedLocationName) {
        ...
        // Utiliser getDivisionName pour récupérer le nom depuis l'API INSTAT

        const divisionName = await getDivisionName(businessData.companyInfo.divisionCode);

        setCompanyLocationName(divisionName);
  ...
  const validateStep = (): string | null => {
    ...
    if (currentStep === 2) {
      ...
      if (personal.isMarried === true) {
        ...
        if (!personal.nombreConjoints || personal.nombreConjoints < 1) {
          ...
          return 'Le nombre de conjoints doit être renseigné.';
        ...
        if (!personal.conjoints || personal.conjoints.length === 0) {
          ...
          return 'Les informations des conjoints doivent être renseignées.';

        }

        

        // Vérifier chaque conjoint (l'acte de mariage sera validé au Step 4)

        for (let i = 0; i < personal.conjoints.length; i++) {

          const conjoint = personal.conjoints[i];

          const conjointLabel = `Conjoint(e) ${i + 1}`;

          

          if (!conjoint.prenom) return `${conjointLabel}: Prénom obligatoire`;

          if (!conjoint.nom) return `${conjointLabel}: Nom obligatoire`;

          if (!conjoint.dateMariage) return `${conjointLabel}: Date de mariage obligatoire`;

          if (!conjoint.lieuMariage) return `${conjointLabel}: Lieu de mariage obligatoire`;

          if (!conjoint.regimeMatrimonial) return `${conjointLabel}: Régime matrimonial obligatoire`;

          if (!conjoint.clauseRestrictive) return `${conjointLabel}: Clause restrictive obligatoire`;
    ...
    if (currentStep === 4) {
      ...
      participants.forEach((p) => {
        ...
        if (requiresManagerDocuments && businessData.personalInfo?.isMarried === true && businessData.personalInfo?.conjoints && businessData.personalInfo.conjoints.length > 0) {
  ...
  // Function to check if we can proceed to next step - useMemo pour réactivité
  ...
  const canProceedToNextStep = useMemo(() => {
    ...
    switch (currentStep) {
      ...
      case 2:
        ...
        console.log('🔍 [VALIDATION STEP 2] Données personnelles:', {
          ...
          divisionId: personal?.divisionId,

          isMarried: personal?.isMarried,

          conjoints: personal?.conjoints
        ...
        if (!personal?.firstName || !personal?.lastName || !personal?.birthDate || !personal?.birthPlace || !personal?.divisionId) {

          console.log('❌ [VALIDATION STEP 2] Champs de base manquants');
        ...
        if (personal.isMarried === true) {
          ...
          console.log('🔍 [VALIDATION STEP 2] Personne mariée, validation des conjoints...');

          

          // Vérifier qu'il y a au moins un conjoint

          if (!personal.conjoints || personal.conjoints.length === 0) {

            console.log('❌ [VALIDATION STEP 2] Aucun conjoint trouvé');
          ...
          // Vérifier que chaque conjoint a tous les champs obligatoires remplis

          // NOTE: L'acte de mariage sera uploadé au Step 4, pas au Step 2

          for (let i = 0; i < personal.conjoints.length; i++) {

            const conjoint = personal.conjoints[i];

            console.log(`🔍 [VALIDATION STEP 2] Conjoint ${i + 1}:`, {
            ...
            if (!conjoint.prenom || !conjoint.nom || !conjoint.dateMariage || 

                !conjoint.lieuMariage || !conjoint.regimeMatrimonial || 

                !conjoint.clauseRestrictive) {

              console.log(`❌ [VALIDATION STEP 2] Conjoint ${i + 1} incomplet`);
          ...
          console.log('✅ [VALIDATION STEP 2] Tous les conjoints sont valides');
      ...
      case 4:
        ...
        // Étape 4: Participants - Validation complète
        ...
        // Vérifier qu'il y a exactement un gérant/promoteur
        ...
        // Vérifier la somme des parts = 100%
        ...
        // Vérifier les documents pour chaque participant
        ...
        for (const p of participants) {
          ...
          if (p.civilite === 'PERSONNE_MORALE') {
            ...
            if (!p.rccmFile) return false;

          } else {

            // Documents pour personnes physiques

            if (!p.typePiece || !p.documentFile) return false;

            

            // Documents requis pour gérants/promoteurs
            ...
            if (p.role === 'GERANT' || p.role === 'PROMOTEUR') {
              ...
              // Actes de mariage pour chaque conjoint si marié

              if (businessData.personalInfo?.isMarried === true && businessData.personalInfo?.conjoints && businessData.personalInfo.conjoints.length > 0) {

                for (const conjoint of businessData.personalInfo.conjoints) {
              ...
              if (!p.certificatNationaliteFile && !p.pieceNationaliteFile) {
              ...
              // Certificat de résidence (si non malien)

              const gerantNationality = p.nationnalite || businessData.personalInfo?.nationality || 'MALIENNE';

              if (gerantNationality.toUpperCase() !== 'MALIENNE' && !p.certificatResidenceFile) {
  ...
  }, [currentStep, isForSelf, businessData]);
